...
- [ ] **AR-2**: Implement `tree.json` enumeration over the configured roots. Each entry: `{ relpath, size, mtime_unix_nanos, sha256: Option<String> }`. SHA-256 computed iff `size <= threshold`. Symlinks recorded by target path, not followed. Output via the writer abstraction.
- [ ] **AR-3**: Implement the zip writer. Stage the `.binlog` (verbatim), `tree.json`, and a placeholder empty `imports/` directory into a temp dir, then zip to `<out>/<archive-name>.zip`. Deflate compression.
...
- [ ] **AR-5**: **Integration test** — synthesize a tempdir tree with ~1000 files of varied sizes (some > threshold), invoke the binary, unzip the result, assert: manifest.json round-trips, tree.json contains every file with correct size/mtime, sha256 present iff under threshold, archive filename matches the naming convention.
- [ ] **AR-6**: **Sanitization checkpoint (M1)** — enumerate every field introduced by M1 (`tree.json` relpath / size / mtime / sha256, `manifest.json` machine name / OS / roots / binlog path). Classify each under D-9 (verbatim / redact / drop). Write `crates/msbuild-diagnostic/src/sanitize/rules.rs` skeleton with the per-field decisions encoded as data; add a unit test asserting unknown fields are dropped by default.
...
- [ ] **AR-13**: Add `diff <T1.zip> <T2.zip>` subcommand. Parses both archives' `tree.json` and `manifest.json`; reports unchanged / new / deleted / mtime-only / content-changed per file.
...
- [ ] **AR-15**: Cross-reference: for each "ran because input newer than output" message, look up both files in T1 and T2 `tree.json` and report the actual mtime+sha256 delta. Output a Markdown report through the output abstraction.
- [ ] **AR-16**: **Integration test** — hand-construct T1 and T2 zips with controlled timestamps and a fabricated minimal binlog; assert the diff identifies a touched-but-content-identical file as the cause.
